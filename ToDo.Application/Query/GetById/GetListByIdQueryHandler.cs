...
{
    public class GetListByIdQueryHandler : IRequestHandler<GetByIdQuery, TodoItem>
    {
        ...
        public async Task<TodoItem> Handle(GetByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            result = await _toDoRepository.SingleOrDefaultAsync(t => t.Id == request.Id);
            if (result == null)
            {
                throw new NotFoundExeption(new { request.Id });
            }
            else if (_currentUserId == result.OwnerId || _userRoles.Contains("Admin"))
            {
